        ...
        <tbody className="w-full">
          {currentData.map((item, index) => (
            ...
            >
              ...
              <td className="px-4 py-2">
                <span className={`${item.status === 'Approved' ? 'text-green-600' : item.status === 'Rejected' ? 'text-red-600' : 'text-yellow-600'} font-semibold`}>
                  {item.status}
                </span>
              </td>
              <td className="action px-4 py-2 rounded-r-2xl">
                <div className="w-full flex justify-center gap-2 items-center space-x-2">
                  <Button className="!bg-none !border-none" onClick={() => onEdit(item)}>
                    <Edit size={24} color="#FF914D" strokeWidth={3} className="hover:bg-orange-200 overflow-hidden" />
                  </Button>
                  <Button className="!bg-none !border-none" onClick={() => onDelete(item)}>
