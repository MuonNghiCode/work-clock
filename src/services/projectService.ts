...
export const getProjectById = async (id: string): Promise<ResponseModel<ProjectInfo>> => {
    const response = await get(API_CONSTANTS.PROJECT.GET_PROJECT_BY_ID.replace("${id}", id));
    return response as ResponseModel<ProjectInfo>;
}